import torch
import torch.nn as nn
import torch.optim as optim
import time
import os
import numpy as np
import requests
import io
import hashlib
import urllib

from PIL import Image
from torch.utils.data import Dataset, DataLoader
from torchvision import models, transforms

from label_studio_ml.model import LabelStudioMLBase
from label_studio_ml.utils import get_single_tag_keys, get_choice, is_skipped, get_local_path

import albumentations as A  
from albumentations.pytorch import ToTensorV2

from sagemaker.pytorch import PyTorchPredictor
from sagemaker.serializers import JSONSerializer
from sagemaker.deserializers import JSONDeserializer
import sagemaker
import boto3
boto_session = boto3.Session()
s3r = boto3.resource('s3')
sagemaker_session = sagemaker.Session(boto_session=boto_session)


device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')


image_size = 224

image_transforms = A.Compose([A.Resize(224,224),
                            A.Normalize([0.485, 0.456, 0.406],[0.229, 0.224, 0.225]),
                            ToTensorV2()])
image_cache_dir = os.path.join(os.path.dirname(__file__), 'image-cache')
os.makedirs(image_cache_dir, exist_ok=True)


def split_s3_path(s3_path):
    path_parts=s3_path.replace("s3://","").split("/")
    bucket=path_parts.pop(0)
    key="/".join(path_parts)
    return bucket, key

def get_transformed_image(url):
    filepath = get_local_path(url)

    with open(filepath, mode='rb') as f:
        image = Image.open(f).convert('RGB')
        image = np.array(image)

    return image_transforms(image=image)["image"]


class ImageClassifierDataset(Dataset):

    def __init__(self, image_urls, image_classes):
        self.classes = list(set(image_classes))
        self.class_to_label = {c: i for i, c in enumerate(self.classes)}

        self.images, self.labels = [], []
        for image_url, image_class in zip(image_urls, image_classes):
            try:
                image = get_transformed_image(image_url)
            except Exception as exc:
                print(exc)
                continue
            self.images.append(image)
            self.labels.append(self.class_to_label[image_class])

    def __getitem__(self, index):
        return self.images[index], self.labels[index]

    def __len__(self):
        return len(self.images)



class ImageClassifierAPI(LabelStudioMLBase):

    def __init__(self, freeze_extractor=False, **kwargs):
        super(ImageClassifierAPI, self).__init__(**kwargs)
        self.from_name, self.to_name, self.value, self.classes = get_single_tag_keys(
            self.parsed_label_config, 'Choices', 'Image')


        self.modela  = PyTorchPredictor(
            endpoint_name="capstone01-staging",
            sagemaker_session=sagemaker_session,
            serializer=JSONSerializer(),
            deserializer=JSONDeserializer(),
        )


    def predict(self, tasks, **kwargs):
        image_urls = [task['data'][self.value] for task in tasks]
        
        predicted_label_indices = []
        predicted_scores = []

        for img_url in image_urls:
            print(img_url)
            bucket, key = split_s3_path(img_url)
            bucket = s3r.Bucket(bucket)
            object_data = bucket.Object(key)
            response = object_data.get()
            file_stream = response['Body']
            inp_img1 = Image.open(file_stream)
            inp_img1 = np.array(inp_img1)
            inp_img1 = image_transforms(image=inp_img1)["image"]
            input_array1 = {"inputs": inp_img1[None, ...].numpy().tolist()}
            logits = self.modela.predict(input_array1)
            predicted_label_index = np.argmax(logits, axis=1)[0]
            print(predicted_label_index)
            predicted_label_indices.append(predicted_label_index)
            r = np.exp(logits[0])/np.exp(logits[0]).sum()
            predicted_score = r[predicted_label_index]
            predicted_scores.append(predicted_score)
        
        
        predictions = []
        for idx, score in zip(predicted_label_indices, predicted_scores):
            predicted_label = self.classes[idx]
            # prediction result for the single task
            result = [{
                'from_name': self.from_name,
                'to_name': self.to_name,
                'type': 'choices',
                'value': {'choices': [predicted_label]}
            }]

            # expand predictions with their scores for all tasks
            predictions.append({'result': result, 'score': float(score)})

        return predictions

